...
每次 Remotion 成片还会在 `storage/tasks/<task_id>/remotion-<标题slug>-<task8>-<index>/` 生成可独立编辑的项目（可用 `npx remotion studio` 打开；目录名含任务标题与短 task id，便于区分）。可用 WebUI 的 **打开 Remotion 项目** 面板（或 CLI `--remotion-project` / `--remotion-followup`）就地继续该项目并跟进修改。
